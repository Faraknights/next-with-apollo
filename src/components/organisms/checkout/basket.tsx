            ...
            {basket.edges.map((basketCommerce, i) => (
              <div key={basketCommerce.commerce.id}>
                <div className="bg-primary-color-lightened pl-2 rounded text-lg font-medium text-black flex items-center justify-between">
                  ...
                  <div className="bg-white mr-2 text-sm w-5 h-5 rounded-full flex items-center justify-center">
                    {basketCommerce.products.length}
                  </div>
                </div>
                {basketCommerce.products.map((ccProduct, j) => (
                  <div
                    key={ccProduct.product.id}
                    className="grid grid-cols-[auto_1fr_auto] grid-rows-2 my-5"
                  >
                    ...
                    <span className="self-center text-sm">
                      <b className="font-medium text-2xl text-primary-color mr-2">
                        {ccProduct.product.price}€
                      </b>
                      /{getUnitLabel(ccProduct.product.unit)}
                    </span>
            ...
            <div className="flex justify-center">
              <CustomButton
                ...
                onClick={async (e) => {
                  localStorage.setItem(
                    "basket",
                    JSON.stringify({ edges: [] } as Basket)
                  );
                  setBasket({ edges: [] });
                }}
              />
        ...
        <Card>
          <div className="flex flex-col items-center">
            <span className="italic">
              Aucun produit de vos régions dans votre panier
            </span>
      ...
      <CustomButton
        ...
        onClick={(e) => {
          if (login && !login.jwt) {
            ...
            setLoading(true);
            router.push({ pathname: "/commands/time_slot" }, undefined, {
              shallow: true,
            });
