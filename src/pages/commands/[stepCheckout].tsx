...
export default function purchaseProcess(options: purchaseProcess) {
  ...
  const [purchaseProcessPage, setPurchaseProcessPage] = useState(
    PurchaseProcessPage[stepCheckout] as number
  );
  ...
  if (purchaseProcessPage != 0 && login && !login.jwt) {
    router.push({ pathname: "/commands/basket" }, undefined, { shallow: true });
  ...
  useEffect(() => {
    setPurchaseProcessPage(
      PurchaseProcessPage[router.query.stepCheckout.toString()]
    );
    ...
    <Layout title="Mon panier">
      <div className="w-1/2">
        <RadioProgression
          ...
          currentPos={purchaseProcessPage + 1}
        />
      </div>
      ...
      <div
        ...
        style={{
          transform:
            "translateX(-" + (purchaseProcessPage * 100).toString() + "%)",
        }}
      >
        <BasketPage
          basket={basket}
